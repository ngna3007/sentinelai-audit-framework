...
# Import from existing PCI DSS structure
from ..chunking.compliance_standards.pci_dss.extractor import ControlExtractor
from ..chunking.general.recursive_chunking_processor import GeneralRecursiveChunkingProcessor, ChunkingResult
...
try:
    ...
    from output_generators.compliance_standards.pci_dss.csv_generator import PCIDSSCSVGenerator as CSVGenerator
...
class PCIDSSPipelineAdapter:
    ...
    def convert_pdf_to_markdown(self, pdf_path: str, output_path: str) -> bool:
        """
        Convert PDF to markdown using universal PDF converter with PCI processor.
        
        Uses the universal PDF converter with PCI-specific processing.
        """
        try:
            convert_pdf_to_markdown(pdf_path, output_path, processor_type='pci')
            return True
        except Exception:
            return False
    
    def chunk_document(self, markdown_path: str, output_dir: str, chunk_size: int = 512, overlap: float = 0.25, source: str = None) -> ChunkingResult:
        """
        Chunk PCI DSS document using recursive chunking with overlap refinery.
        
        Args:
            markdown_path: Path to the markdown document to chunk
        ...
        Returns:
            ChunkingResult with chunking status and metadata
        """
        try:
    ...
    def get_supported_operations(self) -> List[str]:
        ...
        return ["extract_from_markdown", "generate_csv", "convert_pdf_to_markdown", "chunk_document"]