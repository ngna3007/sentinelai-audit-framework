...
# Temporarily inline the ProductionContentFormatter methods to avoid circular import
class ProductionContentFormatter:
    """Inline copy of ProductionContentFormatter to avoid circular import."""
    ...
    def extract_status_from_content(content: str) -> str:
        ...
        if 'best practice until' in content_lower:
            ...
            cleaned_content = sub(r'_+', ' ', content_lower)
            cleaned_content = sub(r'\s+', ' ', cleaned_content)
            
            date_match = search(r'best practice until.*?(\d{1,2}\s+(march|april|may|june|july|august|september|october|november|december)\s+\d{4})', cleaned_content)
            if date_match:
...
# Temporarily inline SectionExtractor methods to avoid circular import
class SectionExtractor:
    """Inline copy of SectionExtractor to avoid circular import."""
